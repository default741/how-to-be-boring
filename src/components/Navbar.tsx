  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-4xl rounded-full border border-white/10 bg-black/60 backdrop-blur-xl shadow-lg shadow-black/20">
      <div className="flex items-center justify-between px-4 md:px-6 h-14">
        <Link
          href="/"
          className="font-mono text-sm font-bold bg-gradient-to-r from-blue-400 to-violet-400 bg-clip-text text-transparent hover:opacity-80 transition-opacity hidden md:block"
        >
          default741
        </Link>
        <div className="flex items-center w-full md:w-auto overflow-x-auto no-scrollbar">
          <div className="flex gap-1 pr-4 md:pr-0">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  href={item.path}
                  className={`text-xs md:text-sm font-medium px-3 py-1.5 rounded-full transition-all duration-300 whitespace-nowrap ${isActive
